...
export default function SuggestedCarousel() {
  ...
  useEffect(() => {
    const catsDescriptions = new Map()
    const getDescriptions = () => {
      ...
      setDescription(catsDescriptions)
    }
    getDescriptions()
  ...
  const canScrollNext = activeSlide < cats.length - 1
    ...
    <>
      <div className="text-center">
        <div className="flex justify-center gap-4">
          <Link
            className="text-xs underline underline-offset-2 hover:text-lime-300"
            href={"https://github.com/sxidsvit/framer-motion-carousel"}
            target="_blank"
      ...
      <div className="group container mx-6">
        ...
        <div className="relative overflow-hidden">
          <motion.ul
            ...
            drag="x"
            dragConstraints={{
              left: -(FALLBACK_WIDTH * (cats.length - 1)),
            ...
            {cats.map((cat, index) => {
              ...
              return (
                <motion.li
                  ...
                  className={cn(
                    "group  shrink-0 select-none px-3 transition-opacity duration-300",
                    !active && "opacity-30",
                  )}
                  transition={{
                    ease: "easeInOut",
                ...
                >
                    <div
                      className={cn(
                        "grid relative place-content-center overflow-hidden rounded-lg bg-gray-900",
                        active ? "aspect-[5/3]" : "aspect-[4/3]",
                      )}
                    >
                        <Link
                          href={cat.url as string}
                          className="block"
                          target="_blank"
                          rel="noopener noreferrer"
                          draggable={true}
          ...
          <button
            type="button"
            className="group absolute left-[20%] top-1/3 z-20 grid aspect-square place-content-center rounded-full transition-colors"
            style={{
              width: CURSOR_SIZE,
              height: CURSOR_SIZE,
            }}
          ...
          <button
            type="button"
            className="group absolute right-[20%] top-1/3 z-20 grid aspect-square place-content-center rounded-full transition-colors"
            style={{
              width: CURSOR_SIZE,
              height: CURSOR_SIZE,
            }}
